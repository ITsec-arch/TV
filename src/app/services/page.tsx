"use client";
import { Shield, Target, Brain, Code, Cloud, TrendingUp, Eye, FileSearch, Server, type LucideIcon } from "lucide-react";
...
export default function Services() {
  ...
  const Section = ({ title, items }: { title: string; items: ServiceItem[] }) => (
    <section className="mx-auto max-w-7xl px-6 py-12">
      <h2 className="mb-6 text-2xl font-semibold">{title}</h2>
      <div className="grid gap-6 md:grid-cols-3">
        {items.map((x) => (
          <ServiceCard key={x.title} icon={x.icon} title={x.title} description={x.desc} features={x.features} />
        ))}
      </div>
    </section>
  );
